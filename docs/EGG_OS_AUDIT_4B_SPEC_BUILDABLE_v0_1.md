...
- **[L3] Test fixture isolasi cross-file:** race condition transfer.service.test↔dashboard.service.test ditemukan ketika DDL trigger A2 menggeser Vitest worker timing. Pola bahaya: dua test file berbagi UUID prefix dengan entity types berbeda (company≠brand, item≠item). **Namespace registry: 97\*=dashboard, 98\*=approval, fc\*=transfer, af\*/b2\*=wiring, c1\*=audit-list, d1\*=task-service (3B T1), d2\*=task-routes (3B T2).** Aturan: setiap file test baru wajib klaim prefix UUID baru dan mendaftarkannya di tabel ini sebelum commit.
- **[L4] lib/date WIB boundary debt:** `audit/service.ts` filter date range pakai inline `` new Date(`${date}T00:00:00+07:00`) `` + `+86_400_000`. `lib/date` hanya punya helper string→string WIB (`todayWIB`, `monthRangeWIB`); belum ada konverter YYYY-MM-DD→Date UTC. Dashboard pakai SQL `AT TIME ZONE` untuk `timestamptz`. Konsolidasi ke `lib/date` jadi utang — eksekusi saat modul lain butuh pola sama.
